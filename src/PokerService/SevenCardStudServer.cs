...
{
    /// <summary>
    /// A helper based implementation of the 7 card stud server
    /// </summary>
    public class SevenCardStudServer : BaseSevenCardStudEngine
    {
        ...
        /// <summary>
        /// The 7 card stud implementation of a safe player cloner.
        /// </summary>
        /// <param name="safe">The safe player to update</param>
        /// <param name="original">The original player to copy from</param>
        private void setSafePlayerCards(Player safe, Player original)
        {
            for (int i = 0; i < original.Cards.Count; ++i)
            {
                // in 7 card stud only the 1st, 2nd and 7th card are hidden
                if (i == 0 || i == 1 || i == 6)
                    safe.Cards.Add(Card.Empty);
                else // reveal the rest
                    safe.Cards.Add(original.Cards[i]);
            }
        }
